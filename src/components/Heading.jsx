...
export default function Heading({ title, link, to = '/' }) {
    return <>
        <div className="flex flex-wrap   justify-between items-center border-b mb-5 pb-3">
            ...
            {link && <Link to={to} className='text-secondary text-sm flex gap-1 items-end hover:text-primary transition-colors'>
                {link}
                <ArrowRight size={16} />
            </Link>}
        </div>
